...
CommBrickReann::CommBrickReann(LAMMPS *lmp) :
  Comm(lmp),
  ...
  maxsendlist_t(nullptr),
  sendlist_g_send(nullptr),sendlist_g_recv(nullptr),
  sendnum_g(nullptr), recvnum_g(nullptr),
  buf_send(nullptr), buf_recv(nullptr),
  buf_send_t(nullptr), buf_recv_t(nullptr),
  buf_send_i(nullptr), buf_recv_i(nullptr)
{
  style = 0;
...
CommBrickReann::~CommBrickReann()
{
  ...
  //
  if (sendlist_g_send) for (int i = 0; i < maxswap; i++) memory->destroy(sendlist_g_send[i]);
  memory->sfree(sendlist_g_send);
  if (sendlist_g_recv) for (int i = 0; i < maxswap; i++) memory->destroy(sendlist_g_recv[i]);
  memory->sfree(sendlist_g_recv);
  //
  memory->destroy(maxsendlist);
  memory->destroy(maxsendlist_t);

  memory->destroy(buf_send);
  memory->destroy(buf_recv);
  memory->destroy(buf_send_t);
  memory->destroy(buf_recv_t);
  memory->destroy(buf_send_i);
  memory->destroy(buf_recv_i);
}
...
CommBrickReann::CommBrickReann(LAMMPS * /*lmp*/, Comm *oldcomm) : Comm(*oldcomm)
{
  if (oldcomm->layout == Comm::LAYOUT_TILED)
    error->all(FLERR,"Cannot change to comm_style brick_reann from tiled layout");
...
/* ----------------------------------------------------------------------
   initialize comm buffers and other data structs local to CommBrickReann
------------------------------------------------------------------------- */

void CommBrickReann::init_buffers()
{
  multilo = multihi = nullptr;
  cutghostmulti = nullptr;

  buf_send = buf_recv = nullptr;
  buf_send_t = buf_recv_t = nullptr;
  buf_send_i = buf_recv_i = nullptr;
  maxsend = maxrecv = BUFMIN;
  maxsend_t = maxrecv_t = BUFMIN;
  grow_send(maxsend,2);
  grow_send_t(maxsend_t,2);
  memory->create(buf_recv,maxrecv,"comm:buf_recv");
  memory->create(buf_recv_t,maxrecv_t,"comm:buf_recv_t");
  memory->create(buf_recv_i,maxrecv_t,"comm:buf_recv_i");

  nswap = 0;
  maxswap = 6;
  allocate_swap(maxswap);

  nstep_sync_1=0;nstep_sync_2=0;
  sendlist = (int **) memory->smalloc(maxswap*sizeof(int *),"comm:sendlist");
  sendlist_g_send = (int **) memory->smalloc(maxswap*sizeof(int *),"comm:sendlist_g_send");
  sendlist_g_recv = (int **) memory->smalloc(maxswap*sizeof(int *),"comm:sendlist_g_recv");
  memory->create(maxsendlist,maxswap,"comm:maxsendlist");
  memory->create(maxsendlist_t,maxswap,"comm:maxsendlist_t");
  for (int i = 0; i < maxswap; i++) {
    maxsendlist[i] = BUFMIN;
    maxsendlist_t[i] = BUFMIN;
    memory->create(sendlist[i],BUFMIN,"comm:sendlist[i]");
    memory->create(sendlist_g_send[i],BUFMIN,"comm:sendlist_g_send[i]");
    memory->create(sendlist_g_recv[i],BUFMIN,"comm:sendlist_g_recv[i]");
  }
}

/* ---------------------------------------------------------------------- */

void CommBrickReann::init()
{
  Comm::init();

  int bufextra_old = bufextra;
  init_exchange();
  if (bufextra > bufextra_old) {grow_send(maxsend+bufextra,2); grow_send_t(maxsend_t+bufextra,2);}
...
int CommBrickReann::updown(int dim, int dir, int loc,
...
void CommBrickReann::forward_comm(int /*dummy*/)
{
  int n;
  MPI_Request request;
...
void CommBrickReann::reverse_comm()
{
  int n;
  MPI_Request request;
  AtomVec *avec = atom->avec;
  ...
  for (int dim = 0; dim < dimension; dim++) {
    ...
    while (i < nlocal) {
      if (x[i][dim] < lo || x[i][dim] >= hi) {
        ...
        if (nsend > maxsend_t) grow_send_t(nsend,1);
    ...
    if (procgrid[dim] == 1) nrecv = 0;
      ...
      if (nrecv > maxrecv_t) grow_recv_t(nrecv);
...
void CommBrickReann::borders()
{
  ...
  for (dim = 0; dim < 3; dim++) {
    ...
    for (ineed = 0; ineed < twoneed; ineed++) {
        ...
        if (!bordergroup || ineed >= 2) {
          if (mode == Comm::SINGLE) {
            for (i = nfirst; i < nlast; i++)
              if (x[i][dim] >= lo && x[i][dim] <= hi) {
                if (nsend == maxsendlist[iswap]) {grow_list(iswap,nsend);grow_list_t(iswap,nsend);}
      ...
      if (nsend*size_border > maxsend_t) grow_send_t(nsend*size_border,0);
      if (ghost_velocity)
      ...
      if (sendproc[iswap] != me) {
        ...
        if (nrecv*size_border > maxrecv_t) grow_recv_t(nrecv*size_border);
  ...
  if (max > maxsend_t) grow_send_t(max,0);
  max = MAX(maxforward*rmax,maxreverse*smax);
  if (max > maxrecv) grow_recv(max);
  if (max > maxrecv_t) grow_recv_t(max);
...
void CommBrickReann::forward_comm_pair(Pair *pair)
{
  int iswap,n;
  double *buf;
  MPI_Request request;
...
void CommBrickReann::reverse_comm_pair(Pair *pair)
{
  int iswap,n;
  double *buf;
  MPI_Request request;
...
void CommBrickReann::forward_comm_fix(Fix *fix, int size)
{
  int iswap,n,nsize;
  double *buf;
  MPI_Request request;
...
void CommBrickReann::reverse_comm_fix(Fix *fix, int size)
{
  int iswap,n,nsize;
  double *buf;
  MPI_Request request;
...
void CommBrickReann::reverse_comm_fix_variable(Fix *fix)
{
  ...
  for (iswap = nswap-1; iswap >= 0; iswap--) {
    ...
    if (nsend > maxsend_t) grow_send_t(nsend,0);
...
void CommBrickReann::forward_comm_compute(Compute *compute)
{
  int iswap,n;
  double *buf;
  MPI_Request request;
...
void CommBrickReann::reverse_comm_compute(Compute *compute)
{
  int iswap,n;
  double *buf;
  MPI_Request request;
...
void CommBrickReann::forward_comm_dump(Dump *dump)
{
  int iswap,n;
  double *buf;
  MPI_Request request;
...
void CommBrickReann::reverse_comm_dump(Dump *dump)
{
  int iswap,n;
  double *buf;
  MPI_Request request;
 ...
 *
*/
void CommBrickReann::set_sendlistg(/*std::unordered_map<int, int>atom_map*/)
{
  int n;
  auto natoms=atom->natoms;
  auto nlocal=atom->nlocal,nghost=atom->nghost;
  tagint *tag = atom->tag;
  MPI_Request request;
  ...
  int nsize=1;
  for (int iswap = 0; iswap < nswap; iswap++) {
    int nummax=sendnum[iswap]>recvnum[iswap]?sendnum[iswap]:recvnum[iswap];
    nsize=nummax>nsize?nummax:nsize;
    if(nummax>maxsendlist_t[iswap]) grow_list_t(iswap,nummax);
  }

  if (nsize > maxsend_t) grow_send_t(nsize,0);
  if (nsize > maxrecv_t) grow_recv_t(nsize);

  std::unordered_map<int, std::unordered_map<int, int>> atom_unique; 
  for (int iswap = 0; iswap < nswap; iswap++) {
    sendnum_g[iswap]=0;
      if (sendproc[iswap]==me) continue;
    int num=0;
    int sendproc_now=sendproc[iswap];
    if(!atom_unique.count(sendproc_now)){
      atom_unique[sendproc_now][-100]=-100;
    }
    for(size_t isend=0;isend<sendnum[iswap];isend++){
        auto tag_i=tag[sendlist[iswap][isend]];
        if((atom_unique[sendproc_now]).find(tag_i) == (atom_unique[sendproc_now]).end()){
          (atom_unique[sendproc_now])[tag_i]=1;
            sendlist_g_send[iswap][num]=tag_i;
            num+=1;
        }
    }
    sendnum_g[iswap]=num;
    ...
    //for recv
  std::unordered_map<int, std::unordered_map<int, int>> atom_unique_recv; 
  for (int iswap = 0; iswap < nswap; iswap++) {
    recvnum_g[iswap]=0;
      if (sendproc[iswap]==me) continue;
    int num=0;
    int sendproc_now=sendproc[iswap];
    if(!atom_unique_recv.count(sendproc_now)){
      atom_unique_recv[sendproc_now][-100]=-100;
    }
    num=0;
    int recvproc_now=recvproc[iswap];
    for(size_t irecv=firstrecv[iswap];irecv<firstrecv[iswap]+recvnum[iswap];irecv++){
        auto tag_i=tag[irecv];
        if((atom_unique_recv[recvproc_now]).find(tag_i) == (atom_unique_recv[recvproc_now]).end()){
          (atom_unique_recv[recvproc_now])[tag_i]=1;
            sendlist_g_recv[iswap][num]=tag_i;
            num+=1;
        }
    }
   recvnum_g[iswap]=num;
...
/* ----------------------------------------------------------------------
   forward communication of N values in per-atom tensor
------------------------------------------------------------------------- */

void CommBrickReann::forward_comm_tensor(int nsize, std::unordered_map<int, int>atom_map, torch::Tensor &mytensor, int syncflag)
{
  ...
  MPI_Request request;
    ...
    #if MODE_CUDA
  auto device_tensor=mytensor.device();
    #endif

  torch::Tensor shape_t=torch::tensor(mytensor.sizes());
  auto dim_t=shape_t.sizes()[0];//.to(torch::kInt).data_ptr<int>();

  int size_i=(mytensor[0]).numel();

  auto shape_new2=shape_t.data_ptr<long>();
  std::vector<long int> shape_new3(dim_t);
  for (int i=0;i<dim_t;i++) {
    shape_new3[i]=shape_new2[i];
  }
  ...
  for (iswap = 0; iswap < nswap; iswap++) {
    ...
    sendnum_g_max=sendnum_g[iswap]>sendnum_g_max?sendnum_g[iswap]:sendnum_g_max;
    recvnum_g_max=recvnum_g[iswap]>recvnum_g_max?recvnum_g[iswap]:recvnum_g_max;

    int nummax=sendnum_g[iswap]>recvnum_g[iswap]?sendnum_g[iswap]:recvnum_g[iswap];
    nsize=nummax>nsize?nummax:nsize;

  }
  if (nsize*size_i > maxsend_t) grow_send_t(nsize*size_i,0);
  if (nsize*size_i > maxrecv_t) grow_recv_t(nsize*size_i);

if(syncflag==1){
  if (nstep_sync_1>nstep_sync_2){
    /*torch::Tensor*/ id_local_recv_t=torch::empty({nswap,recvnum_g_max},torch::dtype(torch::kLong));
    /*torch::Tensor*/ id_local_send_t=torch::empty({nswap,sendnum_g_max},torch::dtype(torch::kLong));
    for (iswap = 0; iswap < nswap; iswap++) {
  
      if (sendproc[iswap]==me) continue;
  
      //atom_index from global to local 
      torch::Tensor id_local_recv_=torch::empty({recvnum_g[iswap]},torch::dtype(torch::kLong));
      for (int i=0;i<recvnum_g[iswap];i++){
        id_local_recv_[i]=atom_map[sendlist_g_recv[iswap][i]];
      }
      id_local_recv_t.index({iswap,Slice(0,recvnum_g[iswap])})=id_local_recv_;

      torch::Tensor id_local_send_=torch::empty({sendnum_g[iswap]},torch::dtype(torch::kLong));
      for (int i=0;i<sendnum_g[iswap];i++){
        id_local_send_[i]=atom_map[sendlist_g_send[iswap][i]];
      }
      id_local_send_t.index({iswap,Slice(0,sendnum_g[iswap])})=id_local_send_;
  
  
    }
  
      #if MODE_CUDA
      id_local_recv_t=id_local_recv_t.to(device_tensor);
      id_local_send_t=id_local_send_t.to(device_tensor);
      #endif
  
    nstep_sync_2=nstep_sync_1;
  }else{
  }
}
  ...
  for (iswap = 0; iswap < nswap; iswap++) {
    ...
    if (sendproc[iswap]==me) continue;
      torch::Tensor id_local_recv_=id_local_recv_t.index({iswap,Slice(0,recvnum_g[iswap])});
      torch::Tensor id_local_send_=id_local_send_t.index({iswap,Slice(0,sendnum_g[iswap])});

    torch::Tensor mytensor_new_send=(mytensor.index({id_local_send_})).reshape({-1}).contiguous(); 
    #if MODE_CUDA
    mytensor_new_send=mytensor_new_send.cpu();
    #endif

    int mynumel=recvnum_g[iswap]*size_i;
    shape_new3[0]=recvnum_g[iswap];

    // pack buffer
    m = 0;
  #if MODE_FLOAT
    auto myarr=mytensor_new_send.data_ptr<float>();
  #else
    auto myarr=mytensor_new_send.data_ptr<double>();
  #endif

    auto buf_cache=buf_send_t;
    buf_send_t=myarr;
    ...
    if (sendproc[iswap] != me) {
      if (recvnum_g[iswap]) { 
  #if MODE_FLOAT
        MPI_Irecv(buf_recv_t,size_i*recvnum_g[iswap],MPI_FLOAT,recvproc[iswap],0,world,&request);
  #else
        MPI_Irecv(buf_recv_t,size_i*recvnum_g[iswap],MPI_DOUBLE,recvproc[iswap],0,world,&request);
  #endif
      }
      if (sendnum_g[iswap]) {
  #if MODE_FLOAT
        MPI_Send(buf_send_t,size_i*sendnum_g[iswap],MPI_FLOAT,sendproc[iswap],0,world);
  #else
        MPI_Send(buf_send_t,size_i*sendnum_g[iswap],MPI_DOUBLE,sendproc[iswap],0,world);
  #endif
      }
      if (recvnum_g[iswap]) MPI_Wait(&request,MPI_STATUS_IGNORE);
      buf = buf_recv_t;
    } else buf = buf_send_t;
    ...
    if (recvnum[iswap]) { 
  #if MODE_FLOAT
      torch::Tensor mytensor_new_recv=torch::from_blob({buf},shape_new3,torch::dtype(torch::kFloat));
  #else
      torch::Tensor mytensor_new_recv=torch::from_blob({buf},shape_new3,torch::dtype(torch::kDouble));
  #endif
      #if MODE_CUDA
      mytensor_new_recv=mytensor_new_recv.to(device_tensor);
      #endif

      buf_send_t=buf_cache;

      auto mask=id_local_recv_>=atom->nlocal; //!=0;
      mytensor.index_put_({id_local_recv_.index({mask})},mytensor_new_recv.index({mask}));
    }

  }
}

void CommBrickReann::forward_comm_tensor_reduce(int nsize, std::unordered_map<int, int>atom_map, torch::Tensor &mytensor, int syncflag)
{
  int i,j,k,m,iswap,last;
  #if MODE_FLOAT
  float *buf;
...
if(syncflag==1){
  if (nstep_sync_1>nstep_sync_2){
    /*torch::Tensor*/ id_local_recv_t=torch::empty({nswap,sendnum_g_max},torch::dtype(torch::kLong));
    /*torch::Tensor*/ id_local_send_t=torch::empty({nswap,recvnum_g_max},torch::dtype(torch::kLong));
  for (iswap = nswap-1; iswap >= 0; iswap--) {
        if (sendproc[iswap]==me) continue;
    //atom_index from global to local 

    torch::Tensor id_local_recv_=torch::empty({sendnum_g[iswap]},torch::dtype(torch::kLong));
    for (int i=0;i<sendnum_g[iswap];i++){
      id_local_recv_[i]=atom_map[sendlist_g_send[iswap][i]];
    }
    id_local_recv_t.index({iswap,Slice(0,sendnum_g[iswap])})=id_local_recv_;

    torch::Tensor id_local_send_=torch::empty({recvnum_g[iswap]},torch::dtype(torch::kLong));
    for (int i=0;i<recvnum_g[iswap];i++){
      id_local_send_[i]=atom_map[sendlist_g_recv[iswap][i]];
    }
    id_local_send_t.index({iswap,Slice(0,recvnum_g[iswap])})=id_local_send_;
  }
  ...
  for (iswap = nswap-1; iswap >= 0; iswap--) {
      ...
      torch::Tensor id_local_recv_=id_local_send_t.index({iswap,Slice(0,sendnum_g[iswap])});
      torch::Tensor id_local_send_=id_local_recv_t.index({iswap,Slice(0,recvnum_g[iswap])});

    torch::Tensor mytensor_new_send=(mytensor.index({id_local_send_})).reshape({-1}).contiguous(); //view({-1})
    #if MODE_CUDA
    mytensor_new_send=mytensor_new_send.cpu();
    #endif

    int mynumel=sendnum_g[iswap]*size_i;
    shape_new3[0]=sendnum_g[iswap];
    ...
    if (sendproc[iswap] != me) {
      if (sendnum_g[iswap]) { 
  #if MODE_FLOAT
        MPI_Irecv(buf_recv_t,size_i*sendnum_g[iswap],MPI_FLOAT,sendproc[iswap],0,world,&request);
  #else
        MPI_Irecv(buf_recv_t,size_i*sendnum_g[iswap],MPI_DOUBLE,sendproc[iswap],0,world,&request);
  #endif
      }
      if (recvnum_g[iswap]) {
  #if MODE_FLOAT
        MPI_Send(buf_send_t,size_i*recvnum_g[iswap],MPI_FLOAT,recvproc[iswap],0,world);}
  #else
        MPI_Send(buf_send_t,size_i*recvnum_g[iswap],MPI_DOUBLE,recvproc[iswap],0,world);}
  #endif
      if (sendnum_g[iswap]) MPI_Wait(&request,MPI_STATUS_IGNORE);
      buf = buf_recv_t;
    } else buf = buf_send_t;
    ...
    if (sendnum_g[iswap]) { 
      ...
      #endif
      buf_send_t=buf_cache;
      mytensor.index_add_(0,{id_local_recv_},mytensor_new_recv);
    } // if
  } //iswap
}
/* ----------------------------------------------------------------------
...
void CommBrickReann::forward_comm_array(int nsize, double **array)
{
  int i,j,k,m,iswap,last;
  double *buf;
  MPI_Request request;
  ...
  if (nsize > maxforward) {
    ...
    if (maxforward*smax > maxsend_t) grow_send_t(maxforward*smax,0);
    if (maxforward*rmax > maxrecv) grow_recv(maxforward*rmax);
    if (maxforward*rmax > maxrecv_t) grow_recv_t(maxforward*rmax);
  }
...
int CommBrickReann::exchange_variable(int n, double *inbuf, double *&outbuf)
{
...
void CommBrickReann::grow_send(int n, int flag)
{
  if (flag == 0) {
...
void CommBrickReann::grow_send_t(int n, int flag)
{
  if (flag == 0) {
    maxsend_t = static_cast<int> (BUFFACTOR * n);
    memory->destroy(buf_send_t);
    memory->create(buf_send_t,maxsend_t+bufextra,"comm:buf_send_t");
    memory->destroy(buf_send_i);
    memory->create(buf_send_i,maxsend_t+bufextra,"comm:buf_send_i");
  } else if (flag == 1) {
    maxsend_t = static_cast<int> (BUFFACTOR * n);
    memory->grow(buf_send_t,maxsend_t+bufextra,"comm:buf_send_t");
    memory->grow(buf_send_i,maxsend_t+bufextra,"comm:buf_send_i");
  } else {
    memory->destroy(buf_send_t);
    memory->grow(buf_send_t,maxsend_t+bufextra,"comm:buf_send_t");
    memory->destroy(buf_send_i);
...
void CommBrickReann::grow_recv(int n)
{
...
void CommBrickReann::grow_recv_t(int n)
{
  maxrecv_t = static_cast<int> (BUFFACTOR * n);
  memory->destroy(buf_recv_t);
  memory->create(buf_recv_t,maxrecv_t,"comm:buf_recv_t");
  memory->destroy(buf_recv_i);
  memory->create(buf_recv_i,maxrecv_t,"comm:buf_recv_i");
...
void CommBrickReann::grow_list(int iswap, int n)
{
  maxsendlist[iswap] = static_cast<int> (BUFFACTOR * n);
  memory->grow(sendlist[iswap],maxsendlist[iswap],"comm:sendlist[iswap]");
}
void CommBrickReann::grow_list_t(int iswap, int n)
{
  //std::cout<<me<<" "<<iswap<<" grow_list "<<n<<" BUFFACTOR="<<BUFFACTOR<<"\n";
  maxsendlist_t[iswap] = static_cast<int> (BUFFACTOR * n);
  memory->grow(sendlist_g_send[iswap],maxsendlist_t[iswap],"comm:sendlist_g_send[iswap]");
  memory->grow(sendlist_g_recv[iswap],maxsendlist_t[iswap],"comm:sendlist_g_recv[iswap]");
}
...
void CommBrickReann::grow_swap(int n)
{
  free_swap();
  allocate_swap(n);
  if (mode == Comm::MULTI) {
  ...
  sendlist_g_send = (int **)
    memory->srealloc(sendlist_g_send,n*sizeof(int *),"comm:sendlist_g_send");
  sendlist_g_recv = (int **)
    memory->srealloc(sendlist_g_recv,n*sizeof(int *),"comm:sendlist_g_recv");
  memory->grow(maxsendlist,n,"comm:maxsendlist");
  memory->grow(maxsendlist_t,n,"comm:maxsendlist_t");
...
void CommBrickReann::allocate_swap(int n)
{
  memory->create(sendnum,n,"comm:sendnum");
  memory->create(recvnum,n,"comm:recvnum");
  //
  memory->create(sendnum_g,n,"comm:sendnum_g");
  memory->create(recvnum_g,n,"comm:recvnum_g");
  //
...
void CommBrickReann::allocate_multi(int n)
{
...
void CommBrickReann::free_swap()
{
  memory->destroy(sendnum);
  memory->destroy(recvnum);
  //
  memory->destroy(sendnum_g);
  memory->destroy(recvnum_g);
  //
...
void CommBrickReann::free_multi()
{
...
void *CommBrickReann::extract(const char *str, int &dim)
{
  dim = 0;
...
double CommBrickReann::memory_usage()
{
  double bytes = 0;
  bytes += (double)nprocs * sizeof(int);    // grid2proc
  for (int i = 0; i < nswap; i++){
    bytes += memory->usage(sendlist[i],maxsendlist[i]);
    bytes += memory->usage(sendlist_g_send[i],maxsendlist_t[i]);
    bytes += memory->usage(sendlist_g_recv[i],maxsendlist_t[i]);
  }
  bytes += memory->usage(buf_send,maxsend+bufextra);
  bytes += memory->usage(buf_send_t,maxsend_t+bufextra);
  bytes += memory->usage(buf_send_i,maxsend_t+bufextra);
  bytes += memory->usage(buf_recv,maxrecv);
  bytes += memory->usage(buf_recv_t,maxrecv_t);
  bytes += memory->usage(buf_recv_i,maxrecv_t);
  return bytes;
}
